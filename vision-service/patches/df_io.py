...
try:
    ...
    # torchaudio >= 2.x removed torchaudio.backend.common; provide a compat shim.
...
def _sf_info(file: str) -> "AudioMetaData":
    """Return AudioMetaData using soundfile (works with torchaudio >= 2.x)."""
    info = sf.info(file)
    return AudioMetaData(
        sample_rate=info.samplerate,
        num_frames=info.frames,
        num_channels=info.channels,
        bits_per_sample=info.subtype_info.count("16") and 16 or 32,
...
def load_audio(
    ...
    """Loads an audio file using soundfile (torchaudio >= 2.x compat).
    ...
    data, _ = sf.read(file, dtype="float32", always_2d=True)
...
def save_audio(
    ...
    # Use soundfile to write (avoids torchaudio >= 2.x torchcodec requirement)
    audio_np = audio.cpu().numpy()
    if dtype == torch.int16:
        sf.write(outpath, audio_np.T, sr, subtype="PCM_16")
    else:
        sf.write(outpath, audio_np.T, sr, subtype="FLOAT")
